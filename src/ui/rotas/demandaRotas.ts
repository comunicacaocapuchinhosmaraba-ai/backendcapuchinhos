...
router.get('/publicos/:id', async (req: Request, res: Response) => {
  try {
    if (!ObjectId.isValid(req.params.id)) { res.status(404).json({ erro: 'Não encontrado' }); return; }
    const doc = await getCol().findOne({ _id: new ObjectId(req.params.id), status: 'ativo' });
